...
/**
 * 尝试执行,可捕获宕机异常
 * 通用TryCatch方法. 顾名思义,异常catch住之后, 流程会继续往下执行
 * 支持异常堆栈的打印
 * @param fun 目标执行函数
 * @param handler 异常处理函数
 * @return errResult 捕获的异常
 */
func Try(fun func(), handlers ...func(interface{})) (errResult error) {
	defer func() {
		if err := recover(); err != nil {
			err := catch(err.(error), 6) // extend error
			if handlers != nil {
...
// 捕获宕机异常
func catch(err error, offset int) error {
	if err == nil {
		return nil
	}
